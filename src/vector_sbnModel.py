...
class SBN(nn.Module):
    ...
    def __init__(self, taxa, rootsplit_supp_dict, subsplit_supp_dict):
        ...
        for parent in self.subsplit_supp_dict:
            ...
            if ss_len > 1:
                ...
                ss_mask.append(torch.ones(ss_len, dtype=torch.bool))
    ...
    def update_rootsplit_CPDs(self):
        ...
        if torch.isnan(self.rs_CPDs).any():
            raise Exception('Invalid rootsplit probability! Check self.rs:(max {:.4f}, min {:.4f})'.format(np.max(self.CPD_params.detach().numpy()), np.min(self.CPD_params.detach().numpy())))
