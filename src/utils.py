...
def get_tree_list_raw(filename, burnin=0, truncate=None, hpd=0.95, rerooting=False):
    tree_dict = {}
...
def get_tree_list_raw_mrbayes(filename, hpd=0.95, burnin=0.25, translation_d2n=None):
    tree_dict = {}
    tree_wts_dict = defaultdict(float)
    tree_names = []
    i, num_trees = 0, 0
    ...
    logging.info(f"MrBayes {filename} - hpd: {hpd}, burnin: {burnin}")
    ...
    with open(filename, 'r') as input_file:
        while True:
            ...
            if line == "": break
            line = line.split(" ")
            if len(line) > 7 and line[3] == "tree":
                tot_num_trees += 1
    n_burnin = int(tot_num_trees*burnin)
    ...
    with open(filename, 'r') as input_file:
        while True:
            ...
            if len(line) > 7 and line[3] == "tree":
                num_trees += 1
                if num_trees < n_burnin:
                    continue
                tree = Tree(line[7].strip())
                nw_tree = tree.write(format=9)
                tree = ete3.Tree(nw_tree)

                for leaf in tree.get_leaves():
                    leaf.name = translation_d2n[int(leaf.name)]
    ...
    logging.info(f"N-trees: {num_trees}, N-burnin: {n_burnin}")

    tree_wts = [tree_wts_dict[tree_dict[tree_name].get_topology_id()] / (num_trees - n_burnin) for tree_name in tree_names]
    if hpd < 1.0:
        ordered_wts_idx = np.argsort(tree_wts)[::-1]
        cum_wts_arr = np.cumsum([tree_wts[k] for k in ordered_wts_idx])
        cut_at = next(x[0] for x in enumerate(cum_wts_arr) if x[1] > hpd)
        tree_wts = [tree_wts[k] for k in ordered_wts_idx[:cut_at]]
        tree_names_keep = [tree_names[k] for k in ordered_wts_idx[:cut_at]]

    return tree_dict, tree_names, tree_wts


def summary_raw(dataset, file_path, truncate=None, hpd=0.95, runs=range(1,11), goldenrun=False, translation_d2n=None):

    logging.info(f"Loading {list(runs)}")
    ...
    for i in runs:
        if goldenrun:
            tree_dict_rep, tree_names_rep, tree_wts_rep = get_tree_list_raw_mrbayes(file_path + dataset + '/' + f'output_mcmc.run{i}.t', hpd=hpd, translation_d2n=translation_d2n)
        else:
    ...
    run_time += time.time()
    logging.info(f'Support runs {runs} loaded in {run_time} seconds, with {n_samp_tree} sampled trees')
